...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($firstName) || empty($lastName) || empty($email) || empty($password)) {
        ...
        $ipAddress = getUserIpAddr(); // Get the user's IP address
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            $pdo->prepare("INSERT INTO login_attempts (user_id, ip_address, success, timestamp) VALUES (?, ?, 1, NOW())")
                ->execute([$user['id'], $ipAddress]);
            ...
            $pdo->prepare("INSERT INTO login_attempts (user_id, ip_address, success, timestamp) VALUES (?, ?, 0, NOW())")
                ->execute([$user['id'] ?? null, $ipAddress]);
        }
    }
}
?>
